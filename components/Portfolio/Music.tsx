...
const MusicBoard = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="h-90screen w-80screen overflow-y-scroll overscroll-contain rounded-lg bg-[#F1F1F1] dark:bg-gray-700 max-sm:h-80screen sm:w-xl md:w-2xl lg:w-3xl xl:w-4xl"
          onClick={(e) => {
            e.stopPropagation();
          }}
        >
          <div className="flex flex-col">
            <div className="my-10 flex flex-col items-center p-4 text-center">
              <div className="mb-8 text-center text-3xl font-extrabold text-blue-600 dark:text-blue-400">
                {music.title}
              </div>
              <a
                href={music.shareURL}
              ...
              <Musecore
                sheetURL={music.sheetURL}
                musescoreURL={music.musescoreURL}
                title={music.title}
              />
            </div>
          </div>
    ...
    <>
      ...
      >
        <div className="flex flex-col content-center items-center justify-center p-5 md:p-2">
          <div
            style={{ backgroundImage: `url(${music.sampleImg})` }}
            className="mx-auto flex aspect-[3/2] w-full content-center items-center justify-center overflow-hidden rounded-lg bg-white bg-cover bg-center bg-no-repeat dark:bg-black"
          />
          <div className="my-5 break-words md:mb-2">
            <div className="text-center text-lg font-extrabold text-black dark:text-white">
              {music.title}
            </div>
            {music.state === 'popular' && (
              <div className="m-auto w-fit max-w-full text-center text-base font-light">
                <div>
                  Views: {music.views} / Likes: {music.likes}
                </div>
                <div>{music.stars} ⭐</div>
              </div>
            )}
